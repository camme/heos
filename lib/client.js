...
HeosClient.prototype.getGroups = function() {

    var self = this;

    var promise = new Promise(function(resolve, reject) {

        var command = 'player/get_groups';

        var listener = function(data) {
            if (data.payload) {
                var groups = data.payload.map(function(groupInfo) {
                    var group = new Group(self, groupInfo);
                    return group;
                });
                resolve(groups);
                self.emitter.removeListener(command, listener);
...
HeosClient.prototype.getGroup = function(gid) {
